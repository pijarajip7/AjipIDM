#!/usr/bin/env python3
"""
AjipIDM multi-account rotation dashboard (Streamlit).
...
AjipIDM_Batches_*.csv written directly by the EA (AjipIDM_Trade.mqh:
WriteBatchCsv). No MetaTrader5 import here on purpose: this runs as a
...
st.set_page_config(page_title="AjipIDM Rotation Dashboard", layout="wide")
...
def find_batch_csv(files_dir, login):
    if not files_dir or not os.path.isdir(files_dir):
        return None
    matches = glob.glob(os.path.join(files_dir, f"AjipIDM_Batches_*_{login}.csv"))
...
# ------------------------------------------------------------------
# Live panel — only the currently logged-in account has fresh data
# (single-terminal rotation, see README.md "Why a single terminal").
# Refreshes on its own so the rest of the page doesn't need a full
# rerun (and re-read of every batch CSV) just to keep this current.
# ------------------------------------------------------------------
...
# Page
# ------------------------------------------------------------------
st.title("AjipIDM — Multi-Account Rotation Dashboard")
